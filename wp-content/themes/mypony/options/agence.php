...
class AgenceMenuPage {
    ...
    public static function registerScripts ($suffix){
        if ($suffix === 'settings_page_agence_options'){
            ...
            wp_register_script('mypony_admin', get_template_directory_uri() . '/assets/admin.js', ['flatpickr'], false, true);
            wp_enqueue_script('mypony_admin', get_template_directory_uri() . '/assets/admin.js', ['flatpickr'], false, true);
    ...
    public static function registerSettings(){
        ...
        add_settings_field('agence_options_date', "Date d'ouverture", function(){
            ?>
            <input type="text" name="agence_date" value="<?= esc_attr(get_option('agence_date')) ?>" class="mypony_datepicker">
            <?php
        }, self::GROUP, 'agence_options_section');
    }
